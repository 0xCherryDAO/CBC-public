...
class CEX(ABC, Account, CurlCffiClient):
    ...
    def __init__(
        ...
        self.rpc = chain_mapping[self.chain.upper()].rpc
        if proxy:
    ...
    async def deposit(self) -> Optional[bool]:
        is_native = self.token.upper() == chain_mapping[self.chain.upper()].native_token

        chain_symbol = min(
            (k for k, v in chain_mapping.items() if v is chain_mapping[self.chain.upper()]),
            key=lambda k: (' ' in k, len(k), k)
        )
        balance = await self.get_wallet_balance(
            is_native=is_native, address=tokens[chain_symbol][self.token.upper()]
        )
        if balance == 0:
    ...
    async def check_available_chains(self, amount, chains) -> Optional[str]:
        for chain in chains:
            ...
            if balance >= amount:
                logger.debug(f'[{self.wallet_address}] | Нашел баланс в сети: {chain}')
                return chain
