...
def extract_spender(data: str) -> ChecksumAddress:
    params_data = bytes.fromhex(data[10:])
    decoded = decode(["address", "uint256"], params_data)
...
async def create_relay_tx(
    ...
    for transaction in steps:
        if transaction['id'] == 'approve':
            spender = extract_spender(transaction['items'][0]['data']['data'])
            continue
        tx = {
