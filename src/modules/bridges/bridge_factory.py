...
from src.modules.bridges.jumper.jumper_transaction import create_jumper_tx
    ...
    class BridgeClass(ABCBridge):
        ...
        def __str__(self) -> str:
            return (f'{self.__class__.__name__} | [{self.wallet_address}] | '
                    f'[{self.config.from_chain.chain_name} ({self.config.from_token.name}) => '
                    f'{self.config.to_chain.chain_name} ({self.config.to_token.name})]')
...
JumperBridge = create_bridge_class(
    class_name='JumperBridge',
    contract_data=JumperData,
    name='JumperBridge',
    bridge_tx_function=create_jumper_tx
)
